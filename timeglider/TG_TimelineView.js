	...
	*/
	...
	$(CONTAINER)
		...
		.delegate(".tg-legend-all", CLICKORTOUCH, function () {
			...
			var tl = MED.timelineCollection.get($panel.attr("id"));
			var icons = _.pluck(tl.get("legend"), "icon");
...
tg.TG_TimelinePlayer.prototype = {
	...
	displayOutOfFrameEvents: _.throttle(function() {
		...
		// debug.log("left of frame", leftOfFrame);
		
		var rightOfFrame = MED.getFutureEvents(true, true);
		
		// debug.log("right of frame", rightOfFrame);
  ...
  registerTitles: function () {
	...
	buildSettingsMenu: function () {
		...
		$(me._views.SETTINGS_BT).bind(CLICKORTOUCH, function() {			
  		...
  		.find("#timeglider-settings-save").bind(CLICKORTOUCH, function() {
  			// get timezone
			...
			////////////
				...
				*/
				...
				$(".tg-list-li").bind("click", function() {
					// debug.log("myscroll:", tg.listIScroll.distance)
		...
		var me = this,
			...
			switch (modal_type) {
				...
				default:
					...
					var space_above = _sa = co_ht - (Math.abs(ev_top)) -  60;
					var space_below = _sb = Math.abs(ev_top);
